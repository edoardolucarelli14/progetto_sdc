% =========================================================================
% RICEVITORE WLAN 802.11a — ADALM-Pluto SDR
% =========================================================================
% Eseguire questo script sul PC collegato al Pluto RICEVITORE,
% DOPO aver avviato TX_WLAN_Pluto.m sul PC collegato al Pluto TRASMETTITORE.
%
% ADD-ONS RICHIESTI:
%   - WLAN Toolbox
...
%      CONFIGURAZIONE qui sotto (riga 32)
...
frequencyBand  = 2.4;        % Stessa banda del TX
rxGain= 35;                  % Guadagno RX in dB (range: 0–71 dB per Pluto) (35dB con antenne vicine, 58dB distanti)
                             % Aumentare se il segnale è debole

imsize = [76, 102, 3];      % <-- da sostituire con il valore esatto di imsize stampato dal trasmettitore (risoluzione dell'immagine e canali colore (120x160 3 canali RGB)

msduLength = 2304;           % Dimensione massima in byte del payload di ogni pacchetto MDSU (MAC Service Data Unit)
...
captureMultiplier = 11;       % Moltiplicatore lunghezza waveform TX per cattura: determina per quanto tempo la radio rimane in ascolto
% -------------------------------------------------------------------------
...
osf          = 1.5;                         % Over-Sampling Factor: il ricevitore campiona il segnale Wi-Fi a osf*20MHz per migliorare la qualità e prevenire effetto aliasing
sampleRate   = wlanSampleRate(nonHTcfg);    % Estrae il sample rate nominale 20 MHz 

% Numero atteso di MSDUs: calcola quanti byte totali compongono l'immagine
% e divide questo numero per mdsuLenght per sapere quanti pacchetti esatti
% deve aspettarsi di ricevere
totPixels = prod(imsize);
numMSDUs  = ceil(totPixels / msduLength);
...
% Il Wi-Fi si aspetta dati campionati a 20 MHz, ma noi abbiamo registrato a 30 MHz (per via dell'osf = 1.5)
...
[L, M]   = rat(1 / osf);                                            % Calcolo delle frazioni razionali per passare da 30MHz a 20MHz 
maxLM    = max([L M]);
...
b        = designMultirateFIR(L, M, TW, aStop);                     % Creazione del filtro passa_basso antialiasing
...
searchOffset  = 0;  % Puntatatore lettura: spostato in avanti a ogni pacchetto elaborato

ind       = wlanFieldIndices(nonHTcfg);  % Calcolo delle posizioni teoriche di tutti i campioni standard del preambolo 802.11a
Ns        = ind.LSIG(2) - ind.LSIG(1) + 1;  % Lunghezza in campioni in un simbolo OFDM
...
while (searchOffset + minPktLen) <= rxWaveformLen % ciclo while per processare i pacchetti ricevuti in ordine sparso
    ...
    fineFreqOffset = wlanFineCFOEstimate(lltf, chanBW);  % wlanFineCFOEstimate esegue una correzione im frequenza millimetrica usando il campo L-LTF
...
% 2. Stima del canale (Equalizzazione)  
    ...
    format = wlanFormatDetect(nonHT(ind.LLTF(2) + (1:3*Ns), :), ... % Formato stimato utilizzando i 3 simboli OFDM immediatamente successivi al L-LTF
        chanEstLLTF, noiseVarNonHT, chanBW);                        % I simboli contengono l'header specifica della generaione WiFi
...
% 3. Lettura dell'Header (L-SIG)
    ...
    % Lunghezza PSDU (byte)= lunghezza payload — bit2int restituisce int, serve double per wlanNonHTConfig
    ...
    tempInd  = wlanFieldIndices(tempCfg);  % wlanFieldIndices resistutisce la posizione dell'ultimo campione utile
...
% 5. Livello MAC e sequenza
    ...
    [cfgMACRx, msduList{pktInd}, status] = wlanMPDUDecode(rxPSDU, rxNonHTcfg);  % Calcolo del FCS: codice di controllo posizionato in coda al pacchetto, 
                                                                                % se non coincide con quello inviato signfica che il rumore ha invertito almeno un bit
    ...
    if strcmp(status, 'Success')  % Caso ideale: FCS valido
        ...
        packetSeq(pktInd) = bit2int(rxPSDU(seqStart:seqEnd), seqLen, false); % Traduzione da bit a intero
        rxBit{pktInd} = double(msduList{pktInd}); % Pacchetto salvato con posizionamento
    end
    ...
    % in loop se l'array di pacchetti contiene elementi doppi signfica che
    % il trasmettitore sta ricominciando a inviare l'immagine
...
% =========================================================================
% STEP 7: RICOSTRUZIONE DELL'IMMAGINE (CORRETTA PER DATA SHIFT)
% =========================================================================
if ~(isempty(fineTimingOffset) || isempty(pktOffset))
    
    fprintf('\nRicostruzione immagine con allineamento spaziale assoluto...\n');
    
    % 1. Pre-alloca l'array finale con NaN grande quanto l'immagine attesa (salva la geometria dell'immagine)
    ...
    for i = 1:length(packetSeq)  % Per ogni pacchetto ricevuto estrae il Sequence Number
        ...
        % Verifica che il pacchetto appartenga al range previsto per l'immagine
        if seq >= 0 && seq < numMSDUs
            
            % Estrazione e conversione da bit a byte (decimali)
            bits = rxBit{i};
            bits = bits(1 : end - mod(length(bits), 8)); % Tronca la coda dell'array se la sua lunghezza non è un multiplo di 8: eventuali bit spuri / di rimpimento
            bytes = bit2int(reshape(bits(:), 8, []), 8, false)'; % Converte in vettore colonna
            
            % Calcolo degli indici assoluti per il riempimento: 
            startIndex = seq * msduLength + 1; % moltiplica il numero di sequenza per la lunghezza fissa del payload per sapere esattamente in quale pixel inizia il pacchetto in questione
            ...
            % Inserimento esatto dei dati nella "tela"
            numBytes = endIndex - startIndex + 1;
            decData(startIndex:endIndex) = bytes(1:numBytes);  % inserisce i byte nella posizione corretta
        end
    end

    % 3. Generazione e visualizzazione dell'immagine: i NaN verranno convertiti in 0 (pixel neri) dalla funzione uint8
    ...
    if exist('imFig','var') && ishandle(imFig) 
        ...
        imFig.Name = 'Received Image';
    end
    imshow(receivedImage);
    title('Immagine Ricevuta');
    ...
    disp('Impossibile ricostruire l''immagine: nessun pacchetto valido trovato.');
end
